// Main JavaScript for HeadlineHub
...
function getIcon(name) {
  const icons = {
    ...
    userSmall: '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>',
    ...
    return `
      <a href="article.html?id=${article.id}" class="featured-card">
        ...
        <div class="featured-overlay">
          ...
          <div style="display: flex; gap: 1rem; font-size: 0.75rem; color: #e5e7eb;">
...
function createSidebarItem(article) {
  return `
    <a href="article.html?id=${article.id}" class="sidebar-item">
      <img src="${article.image}" alt="${article.title}" class="sidebar-item-image">
      <div class="sidebar-item-content">
        <h4 class="sidebar-item-title">${article.title}</h4>
        <p class="sidebar-item-date">${new Date(article.date).toLocaleDateString()}</p>
      </div>
    </a>
  `;
}
...
function initBreakingNewsTicker() {
  ...
  if (ticker && breakingNews.length > 0) {
    ...
    const content = breakingNews.map(article => 
      `<span class="news-ticker-item">${article.title}</span>`
    ).join('');
